...
/*   Updated: 2020/01/30 16:26:41 by zjamali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	ft_print_dori(t_struct *strc)
{
	if (strc->sign == 1)
...
{
	if (strc->arg.d == 0 && strc->precision == 0)
		strc->lengh = 0;
	if (strc->flag[0] == '-' || strc->flag[1] == '-')
	{
		ft_printsign(strc);
		ft_print_precision(strc);
		ft_putspaces(strc->min_width - strc->lengh - strc->sign,
				&strc->counter);
	}
	else
	{
		...
		{
			ft_printsign(strc);
			ft_putzeros(strc->min_width - strc->lengh - strc->sign,
					&strc->counter);
		}
		else
		{
